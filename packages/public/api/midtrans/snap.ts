...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const gross = Math.round(Number(snapPayload?.grossAmount));
  if (!snapPayload?.orderId || !Number.isFinite(gross) || gross < 1) {
    return res.status(400).json({ error: "Payload tidak valid" });
  }

  const { first_name, last_name } = splitName(String(snapPayload.booking?.guestName ?? ""));

  const snapBody = {
    transaction_details: {
      order_id: snapPayload.orderId,
      gross_amount: gross,
    },
    item_details: [
      {
        id: "BOOKING",
        name: snapPayload.itemName || "Wolio Hills — Menginap",
        price: gross,
        quantity: 1,
      },
    ],
    customer_details: {
      first_name,
      last_name,
      email: snapPayload.booking?.guestEmail || "guest@example.com",
      phone: (snapPayload.booking?.guestPhone || "").replace(/\D/g, "").slice(0, 20) || "0812345678",
    },
